...
public class ListItem extends BodyTagSupport {
	...
	private Boolean hasBindingDatasource() {
	    ...
	    if(pclassname.equals("com.dao.webcontrol.DropdownList")) {
			if(((DropdownList)getParent()).getDatasource() == null)
				return false;
			else 
				return true;
		}
	    if(pclassname.equals("com.dao.webcontrol.RadioButtonList")) {
	...
	private String buildDropdownlistItems(){
		...
		sb.append(MessageFormat.format("<option {1} {2} >{0}</option>", bc.getString(), optionvalue, select));
		return sb.toString();
	}
