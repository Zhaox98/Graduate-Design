				...
				$("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			            ...
			            top.$.dialog.confirm("你确定要注销公告信息?", function(){
				        	window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/noticemanager.do?forwardurl=/admin/noticemanager.jsp&actiontype=delete&id='+id);
				        });
			        });
				});
	...
	<body >
		<div class="search-title">
			...
			<div class="description">
				<a href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load">发布公告</a>
			</div>
		</div>
		<!-- 搜索控件开始 -->
			...
			<form id="searchForm" action="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do" method="post">
				<table cellspacing="0" width="100%">
					<tbody>
						<tr>
							<td>标题 
								<input name="title" value="${title}" class="input-txt" type="text" id="title" /> <input type="hidden" name="actiontype" value="search" /> 
								<input type="hidden" name="seedid" value="${seedid}" /> 
								<input type="hidden" name="forwardurl" value="/admin/noticemanager.jsp" />
								<div class="ui-button">
		...
		<div class="action-details">
			...
			<a id="btnDelete" class="action-btn" href="#"> <em class="icon-delete"></em> <b>删除</b> </a>
		</div>
		<table id="notice" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
			<thead>
				<tr>
					<th>选择</th>
					<th><b>标题</b></th>
			...
			<tbody>
				...
				%>
				<tr>
					<td>&nbsp<input id="chk<%=temnotice.getId()%>" class="check" name="chk<%=temnotice.getId()%>" type="checkbox" 
						value='<%=temnotice.getId()%>'></td>
					<td><%=temnotice.getTitle()%></td>
					<td><%=temnotice.getCreatetime().toLocaleString()%></td>
					<td><%=temnotice.getPubren()%></td>
					<td>
						<a class="orange-href" href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load&id=<%=temnotice.getId()%>&forwardurl=/admin/noticeadd.jsp">
							<i class="icon-edit icon-white"></i>修改
						</a> 
						<a class="orange-href" href="<%=SystemParam.getSiteRoot() %>/admin/noticedetails.jsp?id=<%=temnotice.getId()%>">
