			...
			$(function() {
			    $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			             var id=$(domEle).val();
			             top.$.dialog.confirm("你确定要注销管理员信息?", function(){
				             window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/adminmanager.do?forwardurl=/admin/adminmanager.jsp&actiontype=delete&id='+id);
				         });
			        });
			    });
	 ...
	 <body >
		<div class="search-title">
			...
			<div class="description">
				<a  href="<%=SystemParam.getSiteRoot() %>/admin/adminmanager.do?actiontype=load">新建用户</a>
			</div>
		</div>
		<!-- 搜索控件开始 -->
		<div class="search-options">
			<form id="searchForm"  action="<%=SystemParam.getSiteRoot() %>/admin/adminmanager.do" method="post" >
					...
					<tbody>
						<tr>
					    	<td>用户名
						        ...
						        <input type="hidden"   name="forwardurl" value="/admin/adminmanager.jsp"/>
						        <div class="ui-button">
		...
		<table id="admin" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
			<thead>
				<tr>
					<th><b>用户名</b></th>
					<th><b>姓名</b></th>
			...
			<tbody>
				<c:if test="${listadmin== null || fn:length(listadmin) == 0}">
					<tr>
						<td colspan="20">没有相关管理员用户信息</td>
					</tr>
				</c:if>
				<%
					if(request.getAttribute("listadmin")!=null)
					{
						...
						for(Admin temadmin : listadmin)
						{
				%>
				<tr>
					<td><%=temadmin.getUsername()%></td>
					<td><%=temadmin.getRealname()%></td>
					<td><%=temadmin.getEmail()%></td>
					<td><%=temadmin.getTel()%></td>
					<td>
				   		<a class="orange-href" href="<%=SystemParam.getSiteRoot() %>/admin/admindetails.jsp?id=<%=temadmin.getId()%>">
