			...
			$(function() {
				$("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			       		var id=$(domEle).val();
			         	top.$.dialog.confirm("你确定要注销选课信息?", function(){
				      		window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/xuankemanager.do?actiontype=delete&id='+id);
				      	});
			      	});
			    });
	...
	<body>
		...
		<div class="action-details">
			...
			<a id="btnDelete" class="btn btn-danger"> 
				<i class="icon-trash icon-white"></i> <b>删除${Description}</b>
			</a> 
		</div>
		<table id="module" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
			...
			<tbody>
				...
				<tr>
					<td>&nbsp
						<input id="chk<%=temxuanke.getId()%>" class="check" name="chk<%=temxuanke.getId()%>" type="checkbox"
							value='<%=temxuanke.getId()%>'>
					</td>
					<td><%=temxuanke.getStno()%></td>
					<td><%=temxuanke.getSname()%></td>
					<td><%=temxuanke.getSubno()%></td>
					<td><%=temxuanke.getSubname()%></td>
					<td><%=sdf.format(temxuanke.getXktime())%></td>
					<td><%=temxuanke.getStatus()%></td>
				</tr>
				<% } } %>
			</tbody>
		</table>
