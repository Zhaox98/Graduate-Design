	...
	<body >
					...
					<tbody>
						<tr>
							<td>名称 
								...
								<input type="hidden" name="forwardurl" value="/teacher/classmanager.jsp" />
								<div class="ui-button">
		...
		<table id="banji" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
			...
			<tbody>
				...
				<tr>
					<td>&nbsp
						<input id="chk<%=tembanji.getId()%>" class="check" name="chk<%=tembanji.getId()%>" type="checkbox" value='<%=tembanji.getId()%>'>
					</td>
					<td><%=tembanji.getName()%></td>
					<td>
						<a class="btn btn-info" href="<%=SystemParam.getSiteRoot() %>/admin/classmanager.do?actiontype=load&id=<%=tembanji.getId()%>&forwardurl=/admin/classadd.jsp">
