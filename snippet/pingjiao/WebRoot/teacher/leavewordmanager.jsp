...
<html>
				...
				$("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			           	...
			           	top.$.dialog.confirm("你确定要注销留言信息?", function(){
				        	window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/leavewordmanager.do?forwardurl=/teacher/leavewordmanager.jsp&actiontype=delete&id='+id);
				     	});
			     	});
				});
	...
	<body >
					...
					<tbody>
						<tr>
							<td>标题 
								...
								<input type="hidden" name="forwardurl" value="/teacher/leavewordmanager.jsp" />
								<div class="ui-button">
		...
		<div class="action-details">
			<a id="btnDelete" class="action-btn"> <em class="icon-delete"></em> <b>删除</b> </a>
		</div>
		<table id="leaveword" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
			...
			<tbody>
				...
				<tr>
					<td>&nbsp
						<input id="chk<%=temleaveword.getId()%>" class="check" name="chk<%=temleaveword.getId()%>" type="checkbox" value='<%=temleaveword.getId()%>'>
					</td>
					...
					<td>
						<%
							...
							{
						%>
					    <a class="orange-href" href="<%=SystemParam.getSiteRoot() %>/admin/leavewordmanager.do?actiontype=load&id=<%=temleaveword.getId()%>&tno=<%=temleaveword.getReplyren() %>&forwardurl=/teacher/lwreply.jsp">回复</a>
						<% } %>
					 	<%
					 		if(temleaveword.getStatus()==2)
					 		{
					 	%>
					    <a class="orange-href" href="<%=SystemParam.getSiteRoot() %>/teacher/leaveworddetails.jsp?id=<%=temleaveword.getId()%>">
