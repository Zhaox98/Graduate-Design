			        ...
			        $(".check:checked").each(function(index,domEle){
			        	var id=$(domEle).val();
			      		top.$.dialog.confirm("你确定要注销课程信息?", function(){
				        	window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/coursemanager.do?actiontype=delete&forwardurl=/teacher/coursemanager.jsp&tno=${teacher.tno}&id='+id);
				       	});
			        });
				});
					...
					<tbody>
						<tr>
							<td>课程名
								...
								<input type="hidden" name="forwardurl" value="/teacher/coursemanager.jsp"/>
				...
				<tr>
					...
					<td>
						...
						<form style="display: inline" action="<%=SystemParam.getSiteRoot() %>/admin/coursemanager.do">
							...
							<input type="hidden" name="forwardurl" value="/admin/coursemanager.do?actiontype=get&forwardurl=/teacher/coursemanager.jsp"/>
						...
						<form style="display: inline" action="<%=SystemParam.getSiteRoot() %>/admin/coursemanager.do">
							<input type="hidden" name="actiontype" value="endXuanke"/>
						  	<input type="hidden" name="id" value="<%=temcourse.getId()%>"/>
						   	<input type="hidden" name="forwardurl" value="/admin/coursemanager.do?actiontype=get&forwardurl=/teacher/coursemanager.jsp"/>
							<button class="btn btn-danger"> <i class="icon-zoom-in icon-white"></i>结束选课 </button>
						</form>
						<% } %>
						<a class="btn btn-success" href="<%=SystemParam.getSiteRoot() %>/teacher/coursedetails.jsp?id=<%=temcourse.getId()%>">
